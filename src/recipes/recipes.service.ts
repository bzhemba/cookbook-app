...
export class RecipesService {
  constructor(
    ...
    mapper
      ...
      .forMember(
        ...
        mapFrom((src) =>
          src.createdByUser ? src.createdByUser.username : null,
        ),
      );
  }
  ...
  async getAll(
    ...
    return {
      ...
      links: {
        ...
        previous: prevPage
          ? `/recipes?page=${prevPage}&limit=${limit}`
          : undefined,
  ...
  async filterRecipes(
    ...
    if (ingredients && ingredients.length > 0) {
      queryBuilder.andWhere('ingredients.id IN (:...ingredients)', {
        ingredients,
      });
    }
    ...
    if (maxCookingTime !== undefined) {
      ...
      queryBuilder.andWhere(`${totalTimeQuery} <= :maxTime`, {
    ...
    return {
      ...
      links: {
        ...
        previous: prevPage
          ? `/recipes/filter?page=${prevPage}&limit=${limit}`
          : undefined,
        next: nextPage
          ? `/recipes/filter?page=${nextPage}&limit=${limit}`
          : undefined,
  ...
  async create(createRecipeDto: CreateRecipeDto) {
    const user = await this.userRepository.findOneBy({
      username: createRecipeDto.createdByUser,
    });
    if (!user) {
      throw new NotFoundException(
        `user with username '${createRecipeDto.createdByUser}' does not exist`,
      );
    }

    const recipeImage = await this.imageRepository.findOneBy({
      id: createRecipeDto.imageId,
    });
    if (!recipeImage) {
      throw new NotFoundException(
        `image with id '${createRecipeDto.imageId}' does not exist`,
      );
    }
    ...
    if (!category) {
      throw new NotFoundException(
        `Category with id '${createRecipeDto.categoryId}' does not exist`,
      );
    }
  ...
  async update(
    ...
    if (!user || recipe.createdByUser !== user) {
      throw new NotFoundException(
        `User with name '${username}' can't update this recipe`,
      );
    }
    ...
    if (updateRecipeDto.tags !== undefined) {
      recipe.recipeTags = await Promise.all(
        updateRecipeDto.tags.map(async (tagDto) => {
          const tag = await this.recipeTagRepository.findOneBy({
            id: tagDto.id,
          });
          if (!tag) {
            throw new NotFoundException(
    ...
    if (updateRecipeDto.categoryId !== undefined) {
      ...
      if (!category) {
        throw new NotFoundException(
          `Category with id '${updateRecipeDto.categoryId}' does not exist`,
        );
      }
      recipe.category = category;
    }
    ...
    if (updateRecipeDto.imageData !== undefined) {
      const image = await this.imageRepository.findOneBy({
        id: updateRecipeDto.imageData.id,
      });
      if (!image) {
        throw new NotFoundException(
          `image with id '${updateRecipeDto.imageData.id}' does not exist`,
        );
      }
