...
export class NotesService {
  ...
  async createNote(noteDto: CreateNoteDto): Promise<Note> {
    const user = await this.userRepository.findOneBy({
      username: noteDto.createdByUser,
    });
    if (!user) {
      throw new NotFoundException(
        `user with username '${noteDto.createdByUser}' does not exist`,
      );
    }
  ...
  async updateNote(noteDto: UpdateNoteDto): Promise<Note> {
    ...
    if (!note) {
      throw new NotFoundException(
        `Note with id '${noteDto.id}' does not exist`,
      );
    }
  ...
  async getNotes(username: string): Promise<Note[]> {
    ...
    if (!notes) {
      throw new NotFoundException(
        `Notes created by user '${username}' does not exist`,
      );
    }
    return notes;
  }
